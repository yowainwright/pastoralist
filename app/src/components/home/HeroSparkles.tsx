...
const Star: React.FC<{ size: number; color: string }> = ({ size, color }) => {
  const filter = `drop-shadow(0 0 3px ${color})`;
  const style = { filter };
  return (
    <svg
      width={size}
...
const SPARKLE_PULSE = [0, 1, 0];
const SPARKLE_ROTATION = [0, 135, 0];
const SPARKLE_ANIMATION = {
  opacity: SPARKLE_PULSE,
  scale: SPARKLE_PULSE,
  rotate: SPARKLE_ROTATION,
};

function Sparkle({ left, top, size, color, duration, delay }: (typeof SPARKLES)[number]) {
  const style = { position: "absolute", left, top } as const;
  const transition: Transition = { duration, delay, repeat: Infinity, ease: "easeInOut" };
  return (
    <motion.span
      style={style}
