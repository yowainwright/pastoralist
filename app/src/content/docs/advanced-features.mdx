...
<EpisodeVideo slug="07-sandbox-patches" label="Patch support sandbox demo">
  **Watch the highlighted patch file and dry-run summary:** Pastoralist keeps
  the `lodash@4.17.21` override tied to the same review flow as local patch
  files, so the pin and patch can be reviewed together.
</EpisodeVideo>
...
<EpisodeVideo slug="05-sandbox-cleanup" label="Cleanup sandbox demo">
  **Watch the highlighted stale override list:** Pastoralist previews removal
  for `lodash@4.17.20`, `minimist@1.2.6`, and `qs@6.11.0` because `express` no
  longer needs those pins.
</EpisodeVideo>
...
<EpisodeVideo slug="02-cleanup-unused" label="Unused override cleanup demo">
  **Watch the highlighted cleanup result:** with `--remove-unused --dry-run`,
  Pastoralist shows `lodash@4.17.20`, `minimist@1.2.6`, and `qs@6.11.0` as
  unused overrides before anything is written.
</EpisodeVideo>
