...
<EpisodeVideo slug="11-sandbox-debug" label="Debug output sandbox demo">
  **Watch the highlighted debug line and summary rows:** debug output shows the
  decision trail for `lodash@4.17.20` and `qs@6.11.0`, then warns that one
  override is unused.
</EpisodeVideo>
...
<EpisodeVideo slug="09-sandbox-json-output" label="JSON output sandbox demo">
  **Watch the highlighted JSON fields:** `appliedOverrides`,
  `appendixEntriesUpdated`, and `removedOverridePackages` expose the same result
  as structured data.
</EpisodeVideo>
...
<EpisodeVideo slug="13-sandbox-quiet" label="Quiet mode sandbox demo">
  **Watch the highlighted dry-run update and compact summary:** quiet mode keeps
  the `minimist` and `lodash` output compact while preserving the CI status
  signal.
</EpisodeVideo>
