...
The Pastoralist action is a composite wrapper around
`pastoralist --outputFormat json`. It installs Pastoralist, maps action inputs
to CLI flags, writes step outputs and a job summary, and can optionally create
or update a maintenance PR.
...
| Mode     | Description                                                   |
| -------- | ------------------------------------------------------------- |
| `check`  | Adds `--dry-run` and reports issues without modifying files   |
| `update` | Modifies `package.json` (default); you handle commits         |
| `pr`     | Modifies files and creates or updates a PR when changes exist |
...
In `pr` mode, the action skips the `fail-on-security` and `fail-on-unused`
gates after Pastoralist succeeds so the generated PR can carry the fix.
...
| `dep-paths`         | Workspace package.json patterns or `workspace`                 | -                                          |
...
The output values come from the CLI JSON result:
`hasSecurityIssues`, `hasUnusedOverrides`, `updated`, `securityAlertCount`,
`unusedOverrideCount`, and `overrideCount`.
