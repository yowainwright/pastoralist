...
<EpisodeVideo slug="01-overrides" label="Pastoralist override tracking demo">
  **Watch the highlighted dry-run update and summary rows:** Pastoralist turns
  the `ansi-regex@5.0.1` override into an appendix entry with dependents, the
  review reason, and a kept-by-user note so cleanup will not remove it
  accidentally.
</EpisodeVideo>
