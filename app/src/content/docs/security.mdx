...
>
  **Watch the highlighted security scan and dry-run update:** Pastoralist checks
  the configured provider, reports that 4 packages are protected, and keeps the
  security context connected to the appendix.
</EpisodeVideo>
...
<EpisodeVideo slug="16-sandbox-auto-fix" label="Security auto-fix sandbox demo">
  **Watch the highlighted auto-fix dry run:** Pastoralist checks the older
  `minimist`, `trim`, `lodash`, and `express` ranges, reports that 5 packages
  are protected, and leaves the sandbox unchanged.
</EpisodeVideo>
...
>
  **Watch the highlighted provider setting and scan result:** the `osv` scanner
  choice is explicit, and Pastoralist shows the provider run completed without
  adding overrides.
</EpisodeVideo>
