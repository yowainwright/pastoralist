...
<EpisodeVideo slug="07-sandbox-patches" label="Patch support sandbox demo">
  **Look for \`lodash@4.17.21\` and \`1 override applied\`:** Pastoralist keeps the
  \`lodash\` override tied to the same review flow as local patch files, so the
  pin and patch can be reviewed together.
</EpisodeVideo>
...
<EpisodeVideo slug="05-sandbox-cleanup" label="Cleanup sandbox demo">
  **Look for \`lodash@4.17.20\`, \`minimist@1.2.6\`, \`qs@6.11.0\`, and \`3 stale
  overrides removed\`:** Pastoralist removes those stale pins from the preview
  because \`express\` no longer needs them.
</EpisodeVideo>
...
<EpisodeVideo slug="02-cleanup-unused" label="Unused override cleanup demo">
  **Look for \`lodash@4.17.20\`, \`minimist@1.2.6\`, \`qs@6.11.0\`, and \`Override no
  longer needed\`:** with \`--remove-unused --dry-run\`, Pastoralist shows exactly
  which unused overrides would be deleted before writing.
</EpisodeVideo>
