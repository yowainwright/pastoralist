...
export type SecurityProvider = "osv" | "github" | "snyk" | "npm" | "socket" | "spektion";
...
const isSecurityProvider = (value: unknown): value is SecurityProvider => {
  return isString(value) && ["osv", "github", "snyk", "npm", "socket", "spektion"].includes(value);
};
...
const validateLedger = (value: unknown): boolean => {
  ...
  const securityCheckedValid = isFieldValid(value, "securityChecked", isBoolean);
  const securityCheckDateValid = isFieldValid(value, "securityCheckDate", isString);
  const securityDatesValid = securityCheckedValid && securityCheckDateValid;
  if (!securityDatesValid) return false;

  const securityProviderValid = isFieldValid(value, "securityProvider", isSecurityProvider);
  ...
  const potentiallyFixedInValid = isFieldValid(value, "potentiallyFixedIn", isString);
...
const validateAppendixItem = (value: unknown): boolean => {
  ...
  if ("rootDeps" in value && value.rootDeps !== undefined && !isStringArray(value.rootDeps))
    return false;
  if ("dependents" in value && value.dependents !== undefined && !isRecord(value.dependents))
    return false;
  if ("patches" in value && value.patches !== undefined && !isStringArray(value.patches))
    return false;
  if ("ledger" in value && value.ledger !== undefined && !validateLedger(value.ledger))
    return false;
...
const validateSecurityConfig = (value: unknown): boolean => {
  ...
  if ("provider" in value && value.provider !== undefined && !isSecurityProviders(value.provider))
    return false;
  if ("autoFix" in value && value.autoFix !== undefined && !isBoolean(value.autoFix)) return false;
  if ("interactive" in value && value.interactive !== undefined && !isBoolean(value.interactive))
    return false;
  if (
  ...
  if (
    ...
    return false;
  if ("strict" in value && value.strict !== undefined && !isBoolean(value.strict)) return false;
...
const validatePastoralistConfig = (value: unknown): boolean => {
  ...
  if ("appendix" in value && value.appendix !== undefined && !validateAppendix(value.appendix))
    return false;
  if ("depPaths" in value && value.depPaths !== undefined && !validateDepPaths(value.depPaths))
    return false;
  if (
