...
export const CONFIG_LOCATION_CHOICES: PromptChoice[] = [
  ...
  { name: "In a separate config file (recommended for complex setups)", value: "external" },
];

export const CONFIG_FORMAT_CHOICES: PromptChoice[] = [
  { name: ".pastoralistrc.json (JSON format)", value: ".pastoralistrc.json" },
  { name: "pastoralist.config.js (JavaScript module)", value: "pastoralist.config.js" },
  { name: "pastoralist.config.ts (TypeScript module)", value: "pastoralist.config.ts" },
];

export const WORKSPACE_TYPE_CHOICES: PromptChoice[] = [
  { name: 'Use "workspace" keyword (auto-detect from package.json)', value: "workspace" },
  { name: "Specify custom paths", value: "custom" },
];

export const SECURITY_PROVIDER_CHOICES: PromptChoice[] = [
  { name: "OSV (Open Source Vulnerabilities - free, recommended)", value: "osv" },
...
export const INIT_MESSAGES = {
  ...
  existingConfigWarning: "⚠️  Existing Pastoralist configuration detected. Do you want to overwrite it?",
  existingFileWarning: (filename: string) => `⚠️  ${filename} already exists. Overwrite?`,
...
export const PROMPTS = {
  ...
  enterToken: (provider: string) => `Enter your ${provider} API token (will be stored in config)`,
  securityInteractive: "Enable interactive mode for security fixes? (allows you to review/approve each fix)",
  securityAutoFix: "Enable auto-fix mode? (automatically applies security overrides without prompting)",
