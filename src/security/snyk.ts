...
export class SnykCLIProvider {
  ...
  private convertSnykVulnerabilities(snykResult: any): SecurityAlert[] {
    if (!snykResult.vulnerabilities || !Array.isArray(snykResult.vulnerabilities)) {
      return [];
    }

    return snykResult.vulnerabilities.map((vuln: any) => this.convertVulnToAlert(vuln));
  }

  private convertVulnToAlert(vuln: any): SecurityAlert {
  ...
  private extractPatchedVersion(vuln: any): string | undefined {
    ...
    if (vuln.upgradePath && vuln.upgradePath.length > 1) {
      return vuln.upgradePath[vuln.upgradePath.length - 1].split("@")[1];
    }
