...
import { action, determineSecurityScanPaths } from "../src/program";
...
describe("action", () => {
  ...
  beforeEach(() => {
    ...
    spyOn(scripts, "logger").mockReturnValue(mockLog);
  });
  ...
  it("should merge CLI options with config file settings", async () => {
    ...
    const resolveJSONSpy = spyOn(scripts, "resolveJSON").mockReturnValue(mockConfig);
    const updateSpy = spyOn(scripts, "update").mockResolvedValue();
  ...
  it("should handle errors and exit", async () => {
    ...
    const resolveJSONSpy = spyOn(scripts, "resolveJSON").mockImplementation(() => {
